...
build-and-deploy: ## Build and deploy specific job (usage: make build-and-deploy JOB=job-name)
	@if [ -z "$(JOB)" ]; then echo "Usage: make build-and-deploy JOB=job-name"; exit 1; fi
	@./deployment/scripts/build-and-deploy.sh $(JOB) dev true
