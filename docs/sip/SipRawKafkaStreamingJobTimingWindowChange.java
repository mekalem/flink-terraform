...
public class SipRawKafkaStreamingJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<SipCallEvent> filteredCalls = parsedCalls
            .filter(call -> 
                ...
                // call.from_country_code != null &&  // Using country code field vs from address
                // !call.from_country_code.equals("CA") &&
                // !call.from_country_code.equals("US") &&
        ...
        // filteredCalls.print("FILTERED-CALLS");

        // Then use it:
        filteredCalls
            .map(call -> {
                SimpleDateFormat sdf = getCST_Formatter("yyyy-MM-dd HH:mm:ss.SSS z");
        ...
        windowedCounts
        .map(count -> {
            SimpleDateFormat sdf = getCST_Formatter("yyyy-MM-dd HH:mm:ss z");
            String start = sdf.format(count.windowStart);
            String end = sdf.format(count.windowEnd);
        ...
        env.execute("SIP Call Anomaly Detection Edited Job");
    }
    ...
    public static class SipCallEvent {
        ...
        public String toString() {
            return String.format("SipCall{from=%s, to=%s, from_country=%s, callId=%s}", 
    ...
    // Create a timezone-aware formatter (put this in a helper method or reuse)
    private static SimpleDateFormat getCST_Formatter(String pattern) {
        SimpleDateFormat sdf = new SimpleDateFormat(pattern);
        sdf.setTimeZone(TimeZone.getTimeZone("America/Regina")); // Saskatchewan CST
        return sdf;
    }
}