...
#         # AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, or use IAM role if running in AWS
...
def download_model_from_s3():
    ...
    try:
        ...
        #     print(f"\nFound {len(buckets_response['Buckets'])} bucket(s):\n")
            
        #     for bucket in buckets_response['Buckets']:
        #         print(f"📦 Bucket: {bucket['Name']}")
                
        #         # Try to list contents of each bucket
        #         try:
        #             objects = s3_client.list_objects_v2(Bucket=bucket['Name'], MaxKeys=10)
        #             if 'Contents' in objects:
        #                 print(f"   Contains {len(objects['Contents'])} object(s) (showing first 10):")
        #                 for obj in objects['Contents'][:10]:
        #                     print(f"      - {obj['Key']}")
        #                     if obj['Key'].endswith('.pt'):
        #                         print(f"        ☝️  MODEL FILE!")
        #             else:
        #                 print(f"   (empty)")
        #         except Exception as e:
